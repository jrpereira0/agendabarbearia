...
- No novo agendamento, opção **Cliente sem cadastro**: só o nome (WhatsApp opcional), sem criar ficha em Clientes; a comanda desse horário fica isolada (não agrupa por WhatsApp)
- Na **edição** do agendamento, dá para mudar **data e horário** (não só o horário); se o dia mudar, o horário sai da comanda aberta do dia antigo
...
- **Métricas** (`/admin/metricas`): painel de **análise** por período (`from` / `to`) — visão geral em dois blocos (**Resultado**: faturamento, saídas, lucro líquido; **Operação**: atendimentos, ticket, comissões, produtos, clientes novos vs. recorrentes, ocupação da agenda, cancelamentos), selo de variação vs. período anterior equivalente nos cards principais, evolução diária e métricas detalhadas (incluindo **Saídas**, **Novos vs. recorrentes** por WhatsApp, **Ocupação** da grade e **Cancelamentos** reais — cliente desmarcou / não compareceu; erro de agendamento e remarcação ficam de fora); não é onde se abre/fecha caixa. O endereço antigo `/admin/financeiro` redireciona para cá
