...
export async function updateAppointmentStatus(
  ...
  // Conta de barbeiro sem vínculo em `professionals` (professionalId nulo)
  // não pode alterar status — sem isso o restrictToProfessionalId abaixo
  // vira null e a checagem de dono do horário é pulada (fail-open).
  if (!canViewAllAgendas(session) && !session.professionalId) {
    return { ok: false, error: "Você não pode alterar este agendamento." };
  }
